...
class IncidentsApi(XpanseEndpoint):
    ...
    def list(self, request_data: Any = None, **kwargs: Any) -> XpanseResultIterator:
        ...
        return XpanseResultIterator(api=self._api,
                                    path=f"{self.ENDPOINT}/get_incidents/",
                                    data_key=self.DATA_KEY,
                                    **kwargs)

    def get(self, incident_ids: List[str], request_data: Any = None, **kwargs: Any) -> Any:
        filters = [{"field": "incident_id_list", "operator": "in", "value": incident_ids}]
        kwargs = build_request_payload(request_data=request_data, filters=filters, **kwargs)
        return self._api.post(f"{self.ENDPOINT}/get_incidents/", **kwargs).json()[PublicApiFields.REPLY][self.DATA_KEY]

    def count(self, request_data: Any = None, **kwargs: Any) -> int:
        kwargs = build_request_payload(request_data=request_data, **kwargs)
        return self._api.post(f"{self.ENDPOINT}/get_incidents/", **kwargs)\
            .json()[PublicApiFields.REPLY][PublicApiFields.TOTAL_COUNT]

    def update(self, incident_ids: List[str], update_data: Any, **kwargs: Any) -> bool:
        extra_request_data = {"incident_id_list": incident_ids, "update_data": update_data}
