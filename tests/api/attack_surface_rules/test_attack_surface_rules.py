...
def test_AttackSurfaceRulesApi_list(api):
    ...
    list_request = _api.list(**actual_kwargs)
    assert isinstance(list_request, XpanseResponse)
...
def test_AttackSurfaceRulesApi_get(api):
    ...
    expected_kwargs = {
        DEFAULT_REQUEST_PAYLOAD_FIELD: {
            PublicApiFields.REQUEST_DATA: {
                PublicApiFields.FILTERS: [{"field": "attack_surface_rule_id", "operator": "in", "value": object_ids}],
            },
        },
    }
